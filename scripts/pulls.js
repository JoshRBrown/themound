    ...
    teamProfile.forEach(function(data){
        ...
        if (team === teamName){
            ...
            player.forEach(function(val){
                ...
                var newPlayerInfo = $('<div>').attr('data-player-info-less', '');
                var newH1 = $('<h1>')
                if (playerJersey){
                ...
                // })
                // $(newPlayerInfo).attr('[data-player-info-less]');
                $('[data-team-player]').append(newPlayerInfo);
                $(newPlayerInfo).addClass('player-info-less');
                console.log($('[data-player-info-less]'))

            })
            playerClick($('[data-player-info-less]'));
        }
...
function playerClick(element){
    ...
    // childArr.forEach(function(data){
        ...
        $(element).on('click', function(val){
            ...
            console.log(val)
            // console.log(val.currentTarget.innerText);
            var currentDiv = val.currentTarget.innerText
            if (currentDiv){
                var teamProfile = localStorage.getItem('teamProfile');
                teamProfile = JSON.parse(teamProfile);
                ...
                teamProfile.forEach(function(data){
                    ...
                    allPlayers.forEach(function(loop){
                        console.log(loop['full_name']);
                        // console.log(currentDiv)
                        console.log(currentDiv === loop['full_name'])
                        if (currentDiv == loop['full_name']){
